...
int ExpressionEvaluator::calcPostfix(const std::string &expr) const {
  auto tokens = tokenize(expr);
  std::stack<int> st;
  for (auto &tok : tokens) {
    if (isNum(tok))
...
int ExpressionEvaluator::calcPrefix(const std::string &expr) const {
  auto tokens = tokenize(expr);
  std::stack<int> st;
  for (int i = (int)tokens.size() - 1; i >= 0; --i) {
...
int ExpressionEvaluator::calcInfix(const std::string &expr) const {
